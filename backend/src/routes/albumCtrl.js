...
// 모든 아이돌 앨범 목록 반환
router.get('/album/list', async (req, res) => {
  const con = await db.getConnection();
  try {
    // 모든 앨범 목록 반환
    sql = `SELECT album_id, group_id, name, image_name FROM AlbumData`;
    let [albums] = await con.query(sql);
    return res.status(200).json({ message: '아이돌 앨범 목록 조회에 성공했습니다.', albums });
...
router.post('/album', albumImageUpload.single('image'), verifyLogin, async (req, res) => {
  ...
  try {
    ...
    // DB에 저장
    sql = `
    INSERT INTO AlbumData (name, group_id)
    VALUES ('${name}', ${groupId})`;
...
router.put('/album/:albumId', albumImageUpload.single('image'), verifyLogin, async (req, res) => {
  ...
  try {
    ...
    if (file) {
      // 기존 이미지 삭제
      if (album.image_name) {
        try { fs.rm(path.join(file.destination, album.image_name)); }
        catch (err) { console.error(err); }
      }
...
router.delete('/album/:albumId', verifyLogin, async (req, res) => {
  ...
  try {
    ...
    // 이미지 파일 삭제
    if (album.image_name) {
      try { fs.rm(path.join(ALBUM_IMAGE_DIR, album.image_name)); }
      catch (err) { console.error(err); }
    }
